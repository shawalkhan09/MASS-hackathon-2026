...
## Phase 62 — Two Rounds of Wording Fixes for Intake's Phase 61 Findings: One Resolved, One Confirmed as a Wording-Resistant Limitation

Problem: Phase 61 left both of its findings uncorrected, deliberately, per this project's practice of separating a characterised finding from its fix. This phase attempts the fix for both.

Diagnosis: Round 1 added explicit guidance to `INTAKE_DESCRIPTION` naming both patterns directly, including the two specific phrases ("as a result," "consequently") the causal-link finding had used. Re-running the full 25-trial suite (original evidence preserved in `intake_weak_input_outputs_before_fix/`) showed the quantifier fix worked cleanly: 0 of 25 promotions, no regressions elsewhere. The causal-link fix did not: the raw count stayed at 4 of 25, but every instance moved into a single fixture (WEAK_ENGLISH, up from 3 of 5 to 4 of 5) and switched to unbanned phrasing ("causing X and Y") -- the model was pattern-matching against the two named phrases, not the underlying rule they were meant to stand in for.

Closer reading of WEAK_ENGLISH's failures surfaced a more precise mechanism than "unstated link": the input does state one causal link explicitly ("customer very angry because delivery always late"), and the model was extending that same stated link to also cover a second, separately-stated fact (customer loss) the input never connected to anything -- chaining an existing claim, not inventing one from nothing. Round 2 rewrote the guidance around this specific mechanism: banned causal-connecting language generally rather than two named phrases, and added an explicit rule that a stated link covers only the facts the user connected it to, using the exact WEAK_ENGLISH sentence as the worked example inside the prompt itself. Re-running the full 25-trial suite again (round 1's evidence preserved in `intake_weak_input_outputs_round1_fix/`) produced an unchanged result: still 4 of 5 on WEAK_ENGLISH, several trials reproducing close to the identical sentence the fix had just quoted back to it as the thing not to do, and 0 of 25 everywhere else.

Fix: the quantifier-promotion fix is retained and confirmed stable across two independent 25-trial rounds -- this finding is closed. The causal-link fix is not pursued further by wording alone: two rounds of increasingly specific, example-driven prompt changes, including one that quoted the model's own exact failure back to it as a worked example, produced identical results on the identical fixture. This is treated as evidence of a genuine plateau, not an insufficiently precise instruction, and a third wording round is not attempted here.

Lesson: this is a different shape of result from every other wording fix in this project's history (Check 2, Check 3, and the quantifier fix earlier in this same phase) -- those all responded to a more precisely named rule, sometimes across several rounds, but they moved. This one didn't move on the second attempt at making it more precise, which is itself the useful finding: some failure modes are more reliably addressed by changing what the model is structurally asked to produce (e.g. requiring Background as separate listed facts rather than connected prose, removing the surface opportunity for a causal connector to appear at all) than by further constraining what it is asked not to say. This is scoped as remaining work rather than attempted here, specifically to avoid a third wording round chasing a pattern two rounds have already shown does not respond to that category of fix.

---

## Phase 63 — Check 1 Part B, Round 3: A Reproducible Mechanism Behind the Ohio Warehouse Misses (n=9)

Problem: Phase 55 left the Ohio Warehouse fixture at 5/6 for this round (8/9 pooled with Phase 36's 3/3), explicitly below the project's own n=9-per-fixture standard, with no reproducible link found between the one miss and how the trigger was phrased. Phase 55's individual trial framings and raw per-trial audits were also never saved to disk -- exactly the kind of evidence gap this project's own standard exists to prevent. The report's Section 10.3 committed to a further round using a fresh set of independently-worded framings.

Diagnosis: Built `test_check1_partb_ohio_warehouse_round3.py` as a clean, single-batch, single-variable test: the same Ohio Warehouse case text and the same Root Cause statement held fixed and byte-identical across all 9 trials, varying only the Triggering Event line via mechanical string replacement -- the same discipline `test_check1_trigger_fix.py` used for the restaurant case (Phase 30/31). Rather than 9 more paraphrases of a similar idea, the 9 framings were deliberately designed to span 9 distinct evasion strategies: direct decision-naming (a floor case), vendor-externalising, pure symptom-as-trigger, an agentless passive-voice construction, an event-dated construction structurally mimicking how a genuine external trigger reads in this project's real cases, a vague "operational instability" framing, a downstream-consequence-as-trigger framing, a mechanical/technical-failure register, and a hedged-uncertainty framing. Every trial's full diagnosis and raw, unfiltered Auditor output were saved to disk before any result was tallied, closing the evidence gap Phase 55 left open.

Results: 4 of 9 correctly rejected (trials 1, 4, 6, 7) -- a sharp drop from Phase 36's 3/3 and Phase 55's 5/6. Unlike Phase 55, this round found a reproducible mechanism by reading the Auditor's own PASS reasoning on the 5 misses: in every case, the check substituted "is explicit decision language present in the trigger sentence" for "did the company actually control this." The vendor-externalising framing was passed as "an external technical occurrence resulting from a vendor/system mismatch," disregarding that the company chose the vendor and the rollout. The symptom framing was passed as "the external-facing operational reality the company had to react to," treating a direct effect of the company's own system as something that happened to it. The event-dated framing's own reasoning explicitly conceded "the decision to switch is internal," then passed the trigger anyway on the grounds that the cutover was merely "the temporal marker for the start of the systemic failure, which is acceptable practice" -- the clearest case of the rule being reasoned around in real time, not overlooked. The mechanical-failure framing was passed as "correctly identified as an external/objective event," conflating objective (factually occurred) with external (outside the company's control). The hedged-uncertainty framing was passed because it "is not the company's internal decision-making process" -- treating the absence of decision language as evidence of externality rather than merely the absence of a label. By contrast, all 4 correctly-rejected framings kept an internal actor, action, or transition nameable inside the trigger sentence itself, even when hedged ("had not been fully validated") or softened ("a period of instability that emerged following the transition").

Separately: trial 7 (downstream-consequence framing) also failed Check 3, despite the diagnosis body (Fishbone, 5 Whys, Financial Impact -- identical text in all 9 trials) passing Check 3 clean in every other trial. This is a same-input, different-verdict result on a check unrelated to Part B -- logged as a Check 3 non-determinism data point, not folded into this phase's Part B finding.

Fix: None applied. Per this project's established practice of separating a characterised finding from its fix (Phase 61/62), this phase's scope is characterising the mechanism at the project's own n=9 standard, not yet correcting Check 1 Part B's wording to close it. The report's Section 8.15 documents this finding in full, Section 10.2 records it as a confirmed limitation, and Section 10.3's Ohio Warehouse remaining-work item is updated from "run a further round" (now done) to "fix the identified substitution mechanism, then re-validate at the same n=9 scale."

Lesson: report at exactly the scale run, and do not let a pooled ratio flatten a real pattern. This round's 4/9 stands on its own -- it is not summed with Phase 36 (3/3) and Phase 55 (5/6) into a single 12/18 figure, because that figure would erase the bimodal, mechanism-differentiated result these 9 deliberately-designed framings exist to surface. Check 1 Part B is not evenly robust to rewording: it holds when an internal actor or action stays nameable in the trigger sentence, and fails when a framing routes around that naming via a third party, a symptom, a bare dated event, an "objective" register, or an epistemic hedge. This is a more precise, more actionable characterisation than Phase 55 could produce, and gives any future fix attempt a concrete mechanism to target rather than an unexplained miss rate to chase.

---

## Phase 64 — Fidelity Check n=9 Re-Validation: A Major Reversal on `peloton_real` / Check C

Problem: `fidelity_check.py`'s isolated Check A/B/C architecture (Phase 34) has only ever been verified at n=3, and only against 2 of its 5 established fixtures (`peloton_real`, `category_renamed`) -- the module's own docstring says plainly this "needs its own verification pass, same standard as every prior revision in this file's history." The report's Section 10.2 also flags the fidelity check as validated at "a considerably smaller scale (n=3 to n=5)" than the Auditor's n=9 standard.

Diagnosis: Built `test_fidelity_check_n9_revalidation.py`, re-running all 5 established fixtures unchanged (no new fixture construction) at n=9, calling only the check(s) each fixture actually targets except `peloton_real`, which gets all three since it is the project's one real, multi-cause fixture. 63 LLM calls total. Results:

- `paraphrase_only` / Check A: 9/9 PASS (expected PASS)
- `parenthetical_dropped` / Check B: 9/9 PASS (expected PASS)
- `category_renamed` / Check B: 9/9 FAIL (expected FAIL)
- `qualifier_dropped` / Check C: 9/9 FAIL (expected FAIL)
- `peloton_real` / Check A: 9/9 PASS (expected PASS)
- `peloton_real` / Check B: 9/9 PASS (expected PASS)
- `peloton_real` / Check C: **1/9 FAIL** (expected FAIL)

Six of seven fixture/check combinations replicate perfectly -- 54/54 individual trials matching expectation, the largest clean batch this project has run on the fidelity check by a wide margin. The seventh is a serious reversal: Phase 34's own "Verification, round 2" reported `peloton_real` / Check C coming back FAIL in all 3 trials, called at the time "the cleanest result across all five fidelity-check revisions to date." At n=9, run against the identical, unchanged fixture pair and unchanged Check C wording, it FAILs correctly in only 1 of 9 trials. The other 8 all return "Instances found: None found," several explicitly asserting (incorrectly) that specific compositional qualifiers were preserved when the report text demonstrably drops them.

The one correct catch (trial 8) is a strictly *better* result than anything previously documented for this fixture: it names four distinct dropped qualifiers, not the two originally found in the graduation run -- the "storage, insurance, obsolescence" composition dropped from the carrying-cost figure, the "(Precor + Factory)" composition dropped from the $800M figure, the "into expansion" scope dropped from the 55%-of-capital-invested figure, and the "80% Decline" qualifier dropped from the $40B shareholder-value figure. This confirms the violations are real and detectable in principle -- Check C simply fails to detect them in 8 of 9 independent attempts against the exact same input.

By contrast, `qualifier_dropped` -- the synthetic fixture built to isolate exactly one clean qualifier drop in an otherwise short, single-paragraph-based report -- caught its violation 9/9 times, at this same n, under this same unchanged check wording, run in this same batch. The two fixtures test the identical failure category (a specific compositional/scope qualifier replaced with a generic substitute) and get opposite reliability. The leading, unconfirmed hypothesis: Check C's near-perfect performance on a minimal single-variable synthetic fixture may not generalize to a real, denser report carrying multiple simultaneous violations among many correctly-preserved figures -- a generalization question distinct from, and not addressed by, any of the five wording-calibration rounds in Section 6.7, all of which were validated primarily against synthetic single-variable fixtures.

Two possible confounds are logged honestly rather than resolved: (1) sampling variance at n=3 -- if the true catch rate is near this round's observed ~11% (1/9), a run of 3/3 catches has roughly a 0.1% probability under a naive independent-trials assumption, which is itself informative about how little n=3 can be trusted here; (2) the ~1 month gap between Phase 34's original verification and this round -- `MODEL` ("gemini/gemini-3.1-flash-lite") has not changed in git history, but a hosted model's serving behavior can shift without any visible version change on this project's side. Neither is confirmed; both are logged as open rather than assumed.

Fix: None applied. This is a characterisation phase, consistent with this project's practice of separating a finding from its fix. Given `fidelity_check.py`'s FAIL-CLOSED INTEGRATION RULE (a report failing any of A/B/C is downgraded to FLAGGED_FIDELITY_FAILURE), this result means the fidelity check would have incorrectly approved this exact real report as SYNTHESIZED in roughly 8 of 9 independent runs, despite genuine, multi-instance content drift -- the safety net this component exists to provide would not have fired in the large majority of cases.

Lesson: this is the most consequential result to come out of scaling any of this project's checks to n=9 so far, precisely because it overturns a previously-reported clean result rather than confirming an uncertain one. It is direct, concrete evidence for the general principle this project has stated repeatedly but not previously demonstrated this starkly: a clean result at n=3 is not evidence of reliability, it is an absence of evidence against unreliability, and the two are not the same claim. Report this at its own n=9 scale, not pooled with Phase 34's n=3 -- and treat `peloton_real` / Check C's reliability as an open, high-priority question rather than a settled capability, ahead of any further wording work on the fidelity check.

---

## Phase 65 — PIA/Airlift Forced-Ranking Extended to n=9; a Domain-Dependent Refinement of the Omission-Blindness Finding

Problem: Phase 60's forced-ranking validation on PIA and Airlift was explicitly scoped at n=3 per case -- "corroborating evidence... not an independent replication at the n=9 weight of the original result" -- and the report's Section 10.3 flagged extending it to n=9 as remaining work.

Diagnosis: Built `extend_pia_airlift_to_n9.py` to generate and audit only the 6 *new* runs per case (run4-run9), explicitly refusing to re-run or re-audit the existing 3 real runs already saved from Phase 60, then combining old and new into one real, fully-sourced n=9 tally per case read directly from disk. 24 new LLM calls (12 generation + 12 audit).

Results: all 6 new PIA runs and all 6 new Airlift runs fabricated a percentage-based ranking, and all 12 were correctly caught by the Auditor -- combined with Phase 60's original 3+3, this brings both cases to a clean 9/9 fabricated and 9/9 caught. Check 1 (both parts) passed cleanly across all 18 real trials now on file for these two cases (6 from Phase 60 + 12 new), extending Check 1's validated-at-n=9 set from two cases (Southwest, Peloton, Section 8.7) to four -- directly resolving the exact gap Section 10.3 flagged ("before treating the four-case Check 1 validation... as more than directional").

A secondary finding emerged from reading the new Pareto tables against each case's own stated trigger, not something this script was built to test for: the omission-blindness gap (Phase 24/Section 8.8) recurs with a domain-dependent pattern sharper than Phase 60's smaller sample could show. All 6 new PIA runs omit the stated external trigger (the PK8303 crash) from the ranking table entirely, every time substituting a vague catch-all category ("External Economic/Global Factors," "Miscellaneous (Macro Factors)," "External Regulatory Sanctions (The 'Effect')") at a uniform 5% -- combined with Phase 60's 2 of 3, the crash is omitted in 8 of 9 PIA trials on file. Airlift shows the opposite pattern: the stated trigger (the VC funding winter) appears as its own explicit table row in all 6 new runs -- twice literally parenthetically labeled "(The Trigger)" -- at varying positions (2nd through 4th, 10-25%), consistent with Phase 60's finding that all 3 original Airlift runs also retained it. Combined: present in 9 of 9 Airlift trials on file, omitted in 8 of 9 PIA trials on file. The leading, unconfirmed hypothesis: a discrete, singular event (a plane crash) does not fit naturally into a percentage-weighted structural-cause breakdown and gets abstracted into a generic bucket, while a diffuse macro/environmental condition (a funding winter) reads naturally as one contributing factor among several and keeps its own line. As with every other check in this document, this observation is read from the actual generated text, not inferred from a PASS/FAIL count -- the Auditor's own Check 2 reasoning in all 12 new trials addresses only the fabricated percentages and never once mentions the omission, exactly the same blind spot Section 8.8/8.9 already established.

Fix: None applicable -- this phase is generation and characterisation, not a defect requiring correction. The omission-blindness observation corroborates an already-documented, already-scoped limitation (Section 10.2); it does not open new remaining work beyond what a candidate ranking-completeness check (already in Section 10.3) would address.

Lesson: reaching n=9 did more than tighten a confidence interval -- it surfaced a pattern (the crash/macro-shift asymmetry) that n=3 per case could gesture at but not establish, since 2-of-3 and 3-of-3 are too small a base to distinguish a real domain effect from noise. This is the same shape of value n=9 provided for Check 1 Part B (Section 8.15) and the fidelity check (Section 6.9): more trials did not just confirm what was already believed, they changed what could honestly be said.

---

## Phase 66 — Check 1 Part B Wording Fix Attempted and Reverted: A Longer Instruction Made the Target Problem Worse

Problem: Phase 63 characterised a substitution mechanism behind Check 1 Part B's 4/9 catch rate on fresh Ohio Warehouse framings and left the wording fix itself as remaining work (Section 10.3), to be attempted and validated at n=9 before being trusted.

Diagnosis: Drafted a revised Part B wording explicitly stating "THE TEST IS CONTROL, NOT VOCABULARY" plus five named exclusions matching each of Phase 63's evasion categories, and a companion validation script (target: the same 9 Ohio Warehouse framings; regression: 5 previously-established fixtures). Running it surfaced two things. First, a real bug in the target test script: its trial-running loop lived at module level rather than inside a function guarded by `if __name__ == "__main__":`, so importing it -- which the validation script does, to reuse the identical fixture unchanged -- re-executed the entire original n=9 run as an unplanned side effect, accidentally producing a second, independent n=9 replication under the new wording at the cost of 9 unplanned LLM calls, which then rate-limited two of the intended script's own trials. Second, and more importantly: pooling the accidental replication with the intended run's 7 valid trials (16 total), the new wording caught only 2 of 16 -- both instances of the one framing explicitly named in its own bullet list (vendor-externalising) -- against the Phase 63 baseline of 4/9 (44%). Every other framing, including the direct decision-naming floor case that had never failed under any prior wording, now passed incorrectly. Reading the Auditor's own reasoning across the wrong verdicts shows a consistent new failure mode: the longer wording caused the model to substitute Part A's distinctness test ("is the trigger textually separate from the root cause") for Part B's actual control test -- most starkly in the floor-case trial, whose own reasoning quotes the rule correctly ("the instructions state a company decision is not a valid trigger") and then reasons past it to PASS anyway. The 5 regression fixtures (restaurant, Southwest, Boeing) held at 5/5, so the damage was confined entirely to the subtle framings the fix targeted.

Fix: Reverted `crewai_pipeline.py`'s Part B wording to the original text, confirmed byte-identical to the last committed version via `git diff`. Fixed the test script bug by moving the trial loop into `main()` behind an `if __name__ == "__main__":` guard, so importing its fixtures elsewhere no longer re-executes the run.

Lesson: a longer, more explicit instruction is not the same as a more effective one, and can be actively counterproductive here -- enumerating specific evasion patterns by example appears to have taught the model a closed checklist rather than reinforcing the general principle, so a framing that doesn't match one of the named examples defaulted to a *more* lenient reading than the original, shorter wording produced. This is the same shape of result Phase 62 reached for Intake's causal-link finding: not every characterised limitation responds to being told about more precisely in wording. Per that same practice, this is not pursued into a third wording attempt -- reported as a negative result and reverted, with the Phase 63 characterisation standing as the accurate, current description of Check 1 Part B's behaviour.

---

## Phase 67 — A Structural Fix for Check 1 Part B Was Also Attempted and Reverted: A Correctly-Identified Actor Did Not Bind the Verdict

Problem: Phase 66 reverted a wording-only fix and identified a structural, non-wording approach as remaining work (Section 10.3) -- specifically, requiring the Auditor to state who chose, performed, or could have prevented the named trigger before deciding a verdict, rather than reaching a verdict through free-form justification.

Diagnosis: Changed Part B's required output format so a `Controlling Actor` line must be written before the PASS/FAIL verdict line, naming the specific actor responsible for the triggering event, with the verdict specified to follow directly from that answer rather than from a separate judgment call. No individual evasion pattern was named in the new wording, deliberately avoiding the closed-checklist effect Phase 66 diagnosed. Built `test_check1_partb_structural_fix.py`, re-running the same 9 Ohio Warehouse framings (imported safely from `test_check1_partb_ohio_warehouse_round3.py`, which has an `if __name__ == "__main__":` guard) plus the same 5 regression fixtures Phase 66 used (restaurant x3, Southwest, Boeing) -- this time inlined directly in the new script rather than imported, after discovering `test_check1_trigger_fix.py` has no main-guard of its own and would silently fire 4 extra live Auditor calls as a side effect if imported from. Result: 2 of 9 correctly rejected on target -- below the Phase 63 baseline of 4/9, worse than the fix was meant to produce -- while all 5 regression fixtures held correctly. Reading the Auditor's own reasoning across all 7 misses: the actor-identification step worked exactly as designed in every single case, correctly naming the controlling actor as internal (the company, its leadership, or its IT department) -- it never once mislabelled an internal actor as external. The break is entirely in what followed: having correctly stated the actor was internal, the check still returned PASS, with its stated reasoning applying a different, unrelated test each time -- e.g. "correctly identifies the root cause as the company's own management choices, thus satisfying the audit criteria for avoiding conflation," or "does not attempt to blame an external actor for the trigger." Both restate Part A's distinctness/honest-attribution test, not Part B's control test -- the exact substitution mechanism Phase 63 first characterised, now surviving even with a correctly-answered intermediate field sitting immediately above the verdict it was supposed to constrain.

Fix: Reverted `crewai_pipeline.py`'s Part B wording to the original text, confirmed byte-identical to the last committed version via `git diff` (0 lines). `test_check1_partb_structural_fix.py` and its output directory are kept on disk as this round's evidence, not deleted.

Lesson: this is a more specific negative result than Phase 66's, not a repeat of it -- an explicit, correctly-completed intermediate field did not, by itself, bind the verdict it was instructed to follow from mechanically. A prose instruction describing a dependency between two parts of a required output is not enforced by the model treating that dependency as mechanical; the model can answer the forcing question correctly and still reach its verdict by a separate, unconstrained path. Two prompt-level attempts at this limitation -- one wording-only, one requiring a structured intermediate field -- have now both made the target result worse than the original wording. This is not pursued into a third prompt-only attempt; Phase 63's characterisation stands as Check 1 Part B's accurate, current behaviour. The only remaining path identified is moving actor-to-verdict classification out of the model's own free-form judgment entirely and into deterministic code that parses the stated actor and assigns the verdict directly -- a more substantial change than a further prompt iteration, and scoped as remaining work rather than attempted here.

---

## Phase 68 — The Boeing Externality Open Question, Resolved: Check 1 Part B's Correctness Depends on Whether the Root Cause States a Motive

Problem: Phase 33 raised an open question resting on exactly one real data point -- a single live pipeline run where the trigger was framed as "the erroneous data provided by a single AOA sensor" and Part B FAILed it. Two readings were proposed and left unresolved: (1) Part B's definition of "external" is too narrow to recognize a technical malfunction as distinct from the decision that created vulnerability to it, or (2) Boeing genuinely lacks a clean trigger/root-cause split and `FLAGGED_FOR_REVIEW` is correct behavior. Section 10.3 carried this forward as remaining work.

Diagnosis: Built three sequential n=9 controlled tests, the exact trigger framing from Phase 33 held fixed throughout, root cause varied by round as confounds were discovered. Round 1 (`test_check1_partb_boeing_externality_n9.py`): root cause named the single-sensor decision plus an explicit motive ("in order to minimize pilot retraining costs and preserve the 737 MAX's classification as a variant..."). Result: 5 FAIL / 3 PASS of 8 completed trials (1 lost to a transient 503) -- directly overturning Phase 33's assumption of consistency, which had rested on n=1. The 5 FAIL trials each traced the sensor error back to the company-controlled design choice; the 3 PASS trials evaluated the sensor event in isolation, in near-identical language, without making that connection. But the motive phrase was confirmed (by direct inspection of `cases/Case_02_Boeing_737_MAX_Crisis.md`) to use a specific regulatory-classification framing not stated in those exact terms in the case packet, and Trial 2's Check 3 correctly flagged exactly this -- a genuine confound.

Round 2 (`_round2.py`): removed the motive claim, but restructured the root cause into three items joined by "combined with... and..." (single-sensor design, optional safety features, minimized training). Result: 0 FAIL / 9 PASS of 9 -- a complete reversal. But Check 2 failed 7 of 9 trials, every time reading the bundled list as an unweighted Pareto-style ranking of three causes. This traded one confound for another; the flip could not be attributed to Part B's real behavior.

Round 3 (`_round3.py`): root cause reduced to one decision (the single-sensor design) plus one directly-connected supporting fact (engineers raised concerns internally in 2015-2016 without the design being changed), phrased as a single clause -- no bundling, no motive claim, both facts confirmed directly against the case packet before writing the fixture. Result: 0 FAIL / 9 PASS of 9, clean -- matching round 2's split, not round 1's, with uniform reasoning across all 9 trials, none tracing the malfunction back to the design decision despite it appearing one clause away.

Fix: Not applicable -- this phase is characterization, not a pipeline defect requiring correction.

Lesson: comparing all three rounds' root-cause wording against their results isolates the actual variable -- round 1 is the only one of the three whose root cause states WHY Boeing made the decision; both all-PASS rounds state only WHAT the decision was, whether as a single clause (round 3) or a bundled list (round 2). With the motive stated, Part B correctly connects the malfunction to the decision 5 of 8 times; without it, independently, twice, 9 of 9 times, it reads the same malfunction as free-floating and external despite the causal decision remaining present in the text. This resolves Phase 33's question as neither of its two proposed readings -- not "Part B's definition of external is too narrow," not "Boeing consistently and correctly lacks a clean trigger" -- but a third, more specific finding: Part B's judgment on this case category is not an independent foreseeability test, it is contingent on whether the diagnosis's own prose states the company's motive. This is the same underlying shape of failure Phases 63/66/67 characterized for Ohio Warehouse -- the check responding to a textual cue rather than reasoning about control directly -- now demonstrated on the real case that first raised the question, closing Section 10.3's item with an actual mechanism rather than the two-way open question it was recorded as.

---

## Phase 69 — A Correction to Phase 64's Peloton/Check C Finding: Two Genuine Violations, Not Four, and Three Distinct Failure Mechanisms Behind the Misses

Problem: Phase 64's n=9 re-validation found Check C caught the real Peloton report's dropped qualifiers in only 1 of 9 trials, and characterized the one catch (trial 8) as naming "four distinct dropped qualifiers... confirming the violations are real and detectable in principle." This left the mechanism behind the other 8 misses uncharacterized -- flagged in Section 10.3 as the project's highest-priority open question, with model-behavior drift proposed as one possible, unconfirmed explanation.

Diagnosis: Before designing any new experiment, re-read all 9 already-saved raw Check C trial outputs (`fidelity_check_n9_outputs/peloton_real_CheckC_trial{1-9}`) directly against `peloton_real/report.txt` and `diagnosis.txt` -- zero new LLM calls, confirmed both files unchanged since the Phase 64 run via `git log`. This surfaced two things.

First, trial 8's own four claimed catches don't all hold up. (1) The "(storage, insurance, obsolescence)" composition genuinely never appears in `report.txt` -- confirmed by direct `grep` -- a real, clean catch. (2) The "into expansion" scope genuinely never appears attached to the 55% figure -- "expansion" occurs exactly once elsewhere in the report, in an unrelated SWOT sentence about a strategic pivot -- also a real, clean catch. (3) The "(Precor + Factory)" / $400M+$400M composition called dropped is actually present verbatim in the report -- just in an earlier "Path Taken" narrative section (line 18) rather than the same sentence as the $800M figure being checked (line 44) -- a genuine, unresolved ambiguity about whether Check C's rule requires same-instance or document-wide preservation, not a clean violation. (4) The "80% Decline" qualifier called dropped is present in the report's very next clause -- "...experienced an 80% decline, resulting in a loss of over $40 billion..." is one contiguous sentence -- trial 8 quoted only the back half of it, and its claim is factually incorrect. The fixture has two clean, unambiguous violations, not four.

Second, checking the 8 miss trials specifically against those two real violations surfaces three distinct failure mechanisms, not one generic "failed to notice." Outright hallucination: trials 1 and 5 explicitly assert the report contains "storage, insurance, obsolescence" or a link to "expansion" that direct search confirms is absent. Explicit-but-wrong equivalence judgment: trials 2, 3, 6, and 7 correctly quote the report's actual generic phrasing ("annual carrying cost," "annual overhead") immediately next to the diagnosis's specific composition, then explicitly rule it "synonymous" or scope-preserving anyway. Silent incomplete coverage: 6 of 8 miss trials never check the "into expansion" qualifier at all, addressing other figures instead and concluding "None found" from an incomplete sweep.

Fix: None applicable -- this phase is a correction to an existing characterization via ground-truth verification, not a pipeline defect.

Lesson: a "correct" result deserves the same scrutiny as an incorrect one before being used to support a conclusion. Phase 64 treated trial 8's FAIL verdict as confirmation the fixture's violations were "real and detectable in principle" without checking each of its four named claims against the actual report text, and one of the four was demonstrably wrong. This project's practice of grounding every claim in actual source files applies as much to a check's reasoning as to its PASS/FAIL tally -- the 1-of-9 metric itself remains accurate and unchanged; what needed correcting was the qualitative story built on top of it. The corrected picture is more useful than the original: rather than an unexplained reliability drop with model-behavior drift as the leading unconfirmed hypothesis, there are now three specific, evidenced mechanisms, obtained at zero additional cost by comparing already-saved evidence against files already in this repository, before spending anything on a new experiment.

---

## Phase 70 — Check 4 (Ranking Completeness) Built and Validated at n=18: 9/9 on Airlift, 3/9 on PIA, with a Specific Leniency Mechanism

Problem: Section 10.3 had scoped a candidate fourth Auditor check as a not-yet-attempted scoping decision -- addressing the omission-blindness gap Sections 8.8/8.9/8.14 documented (Check 2 verifies ranking percentages aren't fabricated but never checks whether the ranking is complete, i.e. whether a cause the diagnosis already names elsewhere was silently left out).

Diagnosis: Confirmed first that this needs no change to the Analyst's output -- every diagnosis already labels its Triggering Event and Root Cause explicitly, in the same structure Check 1 already parses reliably. Built `ranking_completeness_check.py` as its own isolated Agent/Task/Crew, mirroring `fidelity_check.py`'s Check A/B/C architecture, specifically to avoid adding a fourth check to Check 1/2/3's shared prompt while whether that shared prompt has the same cross-check dependency the fidelity check had before its Section 6.7 isolation remains an open, untested question (Section 10.3). Check wording deliberately does not enumerate named evasion patterns or example substitute phrases -- Phase 66/67's lesson (naming specific patterns teaches a closed checklist) applied up front rather than re-learned.

Validated against all 18 already-saved PIA/Airlift forced-ranking diagnoses (`repeated_forced_ranking/`, Phase 60/65) -- zero new diagnosis-generation calls, only 18 new Check-4-only calls. Ground truth for each of the 18 confirmed by direct reading of every file before writing the test, not assumed from the Phase 65 prose summary: 8 PIA runs omit the crash from the ranking table entirely (mentioned, if at all, only inside another row's rationale, never as its own row); 1 PIA run (run 2) gives it its own row ("Human Error (Specific Pilot Actions)"); all 9 Airlift runs give their trigger its own row.

Result: 9 of 9 correct on Airlift, 3 of 9 correct on PIA (12 of 18 overall). Reading all 6 PIA mismatches shows one consistent mechanism: in each case, the check finds a row causally connected to the trigger -- its rationale mentions the crash as something that "resulted in," "exposed," or "triggered" something else -- and treats that causal connection as correspondence, rather than requiring the row's own named cause to BE the trigger. Trial 9 accepts a row literally labeled "External Regulatory Sanctions (The 'Effect')" -- the diagnosis's own named effect, not its cause -- as a match. The two correct PIA rejections (trials 6, 7) explicitly refuse exactly this substitution, stating plainly that no corresponding line item exists. Airlift's clean sweep is consistent with the same mechanism, not a separate success: its trigger is already a macro-category noun ("VC winter," "market shift") that maps onto one row nearly directly, giving the check no causally-adjacent rows to mistake for a match, unlike PK8303's discrete narrated event getting referenced in several unrelated rows' background reasoning.

Fix: None applied -- this is a first validation pass on a brand-new check, reported as a characterized candidate rather than pursued into an immediate wording round.

Lesson: a first-pass 12/18 with one precisely-identified, one-directional leniency mechanism is a stronger result than an unexplained failure rate would be, and is in fact cleaner than Check 1 Part B's original 4-of-9 baseline (Section 8.15) was before three further rounds of investigation. Given this project's 0-for-2 record on same-session prompt-level fixes to Check 1 Part B (Sections 8.17, 8.18), a further wording round tightening what "corresponds" means is identified as optional future work rather than attempted immediately -- this result is reported as the candidate check's first characterized behavior, not a closed or final one.