...
## Phase 47 — Wiring Check 3 into the Live Demo UI (simplify_audit_feedback)

Problem: Check 3 has been live in the actual pipeline's AUDIT_DESCRIPTION since its first round, but api.py's simplify_audit_feedback() -- the function that turns raw audit text into the user-facing "why this wasn't verified" reasons shown in the demo UI -- only knew how to parse Check 1 and Check 2's output format. A live demo triggering Check 3 would silently fall back to the generic message rather than surfacing Check 3's actual, specific finding.

Diagnosis/Fix: Added a Check 3 block to simplify_audit_feedback(), matching the same never-raise, degrade-to-generic-fallback philosophy as the existing Check 1/Check 2 blocks. Validated entirely locally against 65 real saved verdict files from every round of this session's Check 3 testing (the true-positive test, rounds 2 and 4, the process-fact stress test, and the n=9 confirmatory replication) -- zero new API calls, since this is pure text parsing rather than a model-behaviour question. Started at imperfect extraction and iterated through five real, confirmed bugs rather than stopping once the code ran without error:
  1. A length cap on quote matching (8-220 chars) was too tight for real claim-quotes, often a full 5-Whys question-and-answer; the regex failed to match the real quote and fell back to pairing unrelated nearby quote marks instead.
  2. Widening the cap broke the opposite direction: short legitimate quotes (e.g. "integration latency", 19 characters) fell below the minimum and were skipped by the pairing regex itself, which shifts every subsequent quote match in the same chunk by one -- the identical failure mode as bug 1, from the other end. Fixed by separating quote-pairing (unbounded) from length-filtering (a distinct step afterward).
  3. Check 3 section extraction ran unbounded to the end of the string, so a model-added bonus paragraph some responses append after Check 3's own structured output (e.g. "### Audit Summary") got scanned too, and its own quoted fragments leaked in. Bounded to stop before the next "###" heading.
  4. FAIL-marker matching was case-insensitive, which matched the ordinary English word "fail" inside a PASS instance's own quoted text (e.g. a 5-Whys question like "why did the synchronization fail..."), pulling that PASS instance's quote into the output as if it were a violation. Fixed to case-sensitive matching, since the model's actual verdict marker is consistently written in uppercase.
  5. A longest-quote-wins heuristic, meant to prefer the real flagged claim over short category labels, backfired when a FAIL instance's own justification quoted the case input's hedge language back as contrast, and that reference quote was longer than the actual violation. Fixed by excluding hedge-shaped quotes ("not established," "still investigating," etc.) before applying the length preference, since a claim that hedges can never be the thing Check 3 exists to flag.
