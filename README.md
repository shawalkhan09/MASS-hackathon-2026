...
- Both Pakistani cases' forced-ranking fabrication result and the Auditor's detection of it were extended from n=3 to n=9 (matching the original three cases' scale), reaching a clean 9/9 fabricated and 9/9 caught for each — see Known Limitations below for the domain-dependent omission-blindness pattern this surfaced.
...
- **No dedicated completeness check exists yet.** An omitted cause or consideration makes no checkable false claim, so it isn't caught by any current check — this would need its own, separately designed and validated check. The gap is domain-dependent: a real cross-case test found a discrete trigger event (a plane crash) omitted from a fabricated ranking in 8 of 9 trials, replaced by a vague catch-all category, while a diffuse trigger (a macroeconomic shift) retained its own line in 9 of 9 — the Auditor's fabrication check never flags either omission, since it only evaluates whether present figures are grounded, not whether the ranking is complete.
- **Check 1 Part B (is the labeled trigger genuinely external, not the company's own decision relabeled) is framing-dependent, not uniformly robust.** A controlled n=9 round of independently-worded framings found it correctly rejects a mislabeled trigger when an internal actor or action stays nameable in the sentence, but is vulnerable to reframing that routes around that naming via a vendor, a symptom, a bare dated event, an "objective" register, or a hedge — 4 of 9 caught. A wording fix targeting this exact mechanism was attempted and made it worse (2 of 16 pooled trials), and was reverted; the limitation stands as characterized, not patched.
- **The Orchestrator's fidelity check has a large, currently unexplained reliability gap on real reports.** Its scope-qualifier check (Check C) holds at 9/9 on a minimal synthetic fixture but caught the same violation in only 1 of 9 trials on the one real, non-synthetic report it's been tested against at this scale — a reversal of an earlier n=3 result that had reported 3/3. Given the fidelity check's fail-closed design, this means a report with genuine content drift could currently be approved as fully verified in most runs. Under active investigation, not yet resolved.
